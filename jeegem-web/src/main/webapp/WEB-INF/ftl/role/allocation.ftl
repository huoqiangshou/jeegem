...
<html>
    ...
    <div id="wrap">
      ...
      <div class="row">
        ...
        <div id="content" class="col-md-12">
          ...
          <!-- page header -->
          <@_pageHeader.pageHeader  '权限管理' '角色分配'/>
          <!-- /page header -->
              ...
              <div class="col-md-12">
                ...
                <section class="tile color transparent-black">
                  ...
                  <div class="tile-body nopadding">
                    ...
                    <form class="form-horizontal" method="post" action="${basePath}/role/allocation.shtml" id="formId" role="form">
                    <input type="hidden" id="selectUserId">
                    <div >
                        <div class="col-sm-3">
                          <input type="text" name="findContent" class="form-control" id="findContent" value="${findContent?default('')}"   placeholder="输入用户昵称 / 用户帐号">
                        </div>
						
						<button type="button" class="btn  btn-amethyst margin-bottom-20" id="searchRoleID">Search。。。</button>
						
						<@shiro.hasPermission name="/role/clearRoleByUserIds.shtml">
							<button type="button" class="btn  btn-red margin-bottom-20" id="deleteAll">清空用户角色</button>
						</@shiro.hasPermission>
                    ...
                    <table class="table">
                      ...
                      <#if page?exists && page.list?size gt 0 >
                        ...
                        <tr>
                          ...
                          <td>${it.nickname?default('-')}</td>
                          <td>${it.email?default('-')}</td>
                           <td>
	                          <#if it.status ==1>
	                          	有效
	                          <#else>
                          ...
                          <td>
								<@shiro.hasPermission name="/role/addRole2User.shtml">
									<a href="/user/chooseRole.shtml?id=${it.id}">选择角色</a>
								</@shiro.hasPermission>
						  </td>
                        </tr>
                        </#list>
 ...
 <div class="modal fade bs-example-modal-sm"  id="selectRole" tabindex="-1" role="dialog" aria-labelledby="selectRoleLabel">
		...
		<section class="tile color transparent-black" id="chooseRoleSection" style="display:none;">
                  <!-- tile header -->
                  <div class="tile-header">
                    <h1><strong>选择角色</strong></h1>
                  </div>
                  <!-- /tile header -->
                  ...
                  <div class="tile-body nopadding">
                    ...
                    <table class="table" id="chooseRoleTable">
				...
				$.post("/role/selectRoleByUserId.shtml",{id:id},function(result){
					if(result && result.length){
						...
						$.each(result,function(){
							...
							html.push("<tr>");
							html.push("<td>");
							html.push("<div class='checkbox check-transparen'>");
							html.push("<input type='checkbox' value='"+(this.id)+"' check='box'  id='box"+(this.id)+"' value= '"+(this.name)+"' >");
							html.push("<label for='box"+(this.id)+"'></label>");
							html.push("</div>");
							html.push("</td>");
							html.push("<td>"+(this.name)+"</td>");
							html.push("</tr>");

						});
						
						return so.id('chooseRoleTable').html(html.join('')) & $('#chooseRoleSection').css('display','block'),$('#selectUserId').val(id),!1;
