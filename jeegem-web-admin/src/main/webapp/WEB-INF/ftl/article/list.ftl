...
<html>
  <head>
    <title>${token.nickname} ---文章列表</title>
    ...
    <div id="wrap">
      ...
      <div class="row">
        ...
        <div id="content" class="col-md-12">
          ...
          <!-- page header -->
          <@_pageHeader.pageHeader  'CMS管理' '文章列表'/>
          <!-- /page header -->
                        ...
                        <#if page?exists && page.list?size gt 0 >
							<#list page.list as item>
		                        <tr>
		                          <td>${item_index+1}</td>
		                          <td>
									  ${item.title?default('未设置')}
				                  </td>
		                          <td>${item.addTime?string('yyyy-MM-dd HH:mm')}</td>
		                          <td>${item.articleType.typeName}</td>
		                          <td>${item.click}</td>
		                          <td><a href="${basePath}/article/deleteById.shtml?id=${item.id}" >删除</a>|<a href="${basePath}/article/editById.shtml?id=${item.id}" >编辑</a></td>
		                        </tr>
	                      	</#list>
                      	</#if>
                      </tbody>
